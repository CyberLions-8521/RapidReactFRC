...
public class Climber extends SubsystemBase {
 ...
 // creating encoder
  ...
  public void Tarzan(XboxController controller){
    m_encoder = ElevatorMotor.getEncoder();
    //if you hit the dpad right and the position is less than x , make it go in one direction
    if (controller.getPOV() == 90 && m_encoder.getPosition() < 144){
      ElevatorMotor.setVoltage(ElevatorOutput.ELEVATOR_UP);
    //if you hit the dpad left and the position is greater than 0, make the motor go in the other direction
    } else if (controller.getPOV() == 270 && m_encoder.getPosition() > 0){
      ElevatorMotor.setVoltage(ElevatorOutput.ELEVATOR_DOWN);
    // if you are NOT pressing the dpap button or the position is greater than x or the position is less than 0 then the motor does nothing
